...
export const INITIAL_APP_STATE: AppState = { status: "idle" };

export function reduceAppState(state: AppState, event: AppEvent): TransitionResult {
  switch (event.type) {
    case "ARM": {
...
function handleSleepStartConfirmed(
  state: Extract<AppState, { status: "armed_waiting_for_sleep" }>,
  sleepStartMs: number,
  detectedAtMs: number,
  alarmId?: AlarmId
): TransitionResult {
  const targetDurationMin: DurationMin = state.targetDurationMin;
...
function getAlarmCancelEffectIfNeeded(state: AppState): ReadonlyArray<StateEffect> {
  if (state.status !== "sleep_detected_alarm_scheduled") {
    return [];
  }
...
function unchanged(state: AppState): TransitionResult {
  return { state, effects: [] };
}

function exhaustiveGuard(unreachable: never): never {
